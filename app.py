...
/* 헤더 */
thead th {
    background: #1e3a6b !important; color: white !important;
    font-weight: 700; border: 1px solid #2d5fa8;
    padding: 7px 10px; text-align: center !important;
    white-space: nowrap; font-size: 13px; vertical-align: middle;
}
thead th.th-sub { background: #2d5fa8 !important; }

/* 모든 데이터 셀: 검정, 동일 사이즈, bold 없음 */
td {
...
/* 셀 내부 모든 요소 강제 통일 */
td *, td span, td small, td b, td strong {
    color: #222 !important;
...
/* 달성률만 색상 유지 (font-weight도 normal) */
.rate-red { color: #c0392b !important; font-weight: normal !important; }
.rate-ok  { color: #1a7a1a !important; font-weight: normal !important; }

/* 구분 라벨 */
.td-label {
    background: #dce6f5 !important; font-weight: 700 !important;
    color: #1e3a6b !important; text-align: center !important;
}
.td-label * { color: #1e3a6b !important; font-weight: 700 !important; }

/* 소분류 라벨 */
.td-sub-label {
    background: #eef2fa !important; font-weight: 600 !important;
    color: #333 !important; text-align: center !important;
}
.td-sub-label * { color: #333 !important; }

/* 줄무늬 */
tr:nth-child(even) td { background: #f8f9fc; }
tr:nth-child(odd) td { background: #ffffff; }
tr td.td-label { background: #dce6f5 !important; }
tr td.td-sub-label { background: #eef2fa !important; }
...
@media print {
    ...
    .upload-box, header, [data-testid='stSidebar'],
...
# 모든 셀: 검정, 동일 사이즈, bold 없음
def cell(당월, 누계):
    return f"{fmt(당월)}<br><small>({fmt(누계)})</small>"

def rate_cell(당월_a, 당월_p, 누계_a, 누계_p):
    return f"{rate_html(당월_a,당월_p)}<br><small>({rate_html(누계_a,누계_p)})</small>"

def inc_cell(당월_a, 당월_p, 누계_a, 누계_p):
    return f"{d_inc(당월_a,당월_p)}<br><small>({d_inc(누계_a,누계_p)})</small>"
...
def load_supply_gsheet():
    try:
        ...
        df['공급량_MJ'] = pd.to_numeric(
            df['공급량_MJ'].astype(str).str.replace(',','').str.replace(' ',''), errors='coerce')
...
df_n2r   = gh_data.get("new_2",{}).get("3_2. 개발량 실적")
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{fmt(개발량_연간)}</td>
      <td>{fmt(개발량_당계)}</td>
      <td>{fmt(개발량_당실) if 개발량_당실 else 입력필요}</td>
      <td>{rate_html(개발량_당실,개발량_당계) if 개발량_당실 else '-'}</td>
      <td>{fmt(개발량_누계)}</td>
      <td>{fmt(개발량_누실) if 개발량_누실 else '-'}</td>
    ...
    <tr>
      ...
      <td>{fmt(총공_연간)}</td>
      <td>{fmt(총공_당계)}</td>
      <td>{fmt(총공_당실_v) if 총공_당실_v else 입력필요}</td>
      <td>{rate_html(총공_당실_v,총공_당계) if 총공_당실_v else '-'}</td>
      <td>{fmt(총공_누계)}</td>
      <td>{fmt(총공_누실_v) if 총공_누실_v else 입력필요}</td>
...
<div class="print-btn-wrap">
    ...
    <div class="print-guide">버튼 클릭 후 → 대상: <b>PDF로 저장</b> → 레이아웃: <b>가로</b> → 저장</div>
</div>
""", unsafe_allow_html=True)
